...
int batchID, nod, nop;
string technology, startDate, endDate, venueDetail, month, trainerName;
...
public:
    ...
    void allotTrainer()
    {
        ...
        while (f2 >> batchID >> technology >> startDate >> endDate >> nod >> venueDetail >> nop >> month)
        {
            ...
            {
                flag = 1;
                f3 << batchID << " " << technology << " " << startDate << " " << endDate << " " << nod << " " << venueDetail << " " << nop << " " << month << " " << trainerName << endl;
            }
        }
